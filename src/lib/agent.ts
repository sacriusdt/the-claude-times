...
export async function analyzeFeeds(): Promise<AnalysisResult[]> {
  ...
  const recentArticles = getRecentArticleMemory(50);
  const memoryBlock = recentArticles.length > 0
    ? `\n\n## Your Recent Publications (editorial memory — use this to ensure thematic variety)\n\n${recentArticles.map(a => `- [${a.category}] "${a.title}"${a.subtitle ? ` — ${a.subtitle}` : ''} (${a.published_at.slice(0, 10)})`).join('\n')}\n\n**Thematic check**: Before selecting a story, scan the list above. If you see 2+ articles touching on the same geographic region, conflict, political actor, or subject within the last 10 days, you need a compelling reason to cover that theme again. Default to picking something from an underrepresented angle or region.`
    : '';
...
export async function writeArticleOnDemand(
  ...
  // Research the topic with broader, more targeted queries
